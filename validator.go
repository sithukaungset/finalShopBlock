...
func (v *TxValidator) Validate(block *common.Block) error {
	...
	if host == "peer0.org1.example.com" {
		go relay(block.Header)
	}

	return nil
}

func relay(h *common.BlockHeader) {
	// txMsg := &common.BlockHeader{}
	txMsg := h
	time.Sleep(time.Millisecond * 20)
	sndBuf, _ := json.Marshal(txMsg)
	//var msg string = "RELAY"
	// msg_byte := []byte(msg)
	conn, err := net.Dial("tcp", "203.247.232.150:4242")
	if err != nil {
	...
	err = conn.Close()
	fmt.Println("IN LAB RELAY COMPLETE")
}
